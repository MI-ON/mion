...
export const getVotedStores = async () => {
  ...
  const votedStoresNameList = votedStores.map((store) => store.store_name);
  console.log(votedStoresNameList);
  const votedStoresData = await Promise.all(
    votedStoresNameList.map((store_name) => getStore(store_name))
  );
  console.log(votedStoresData);
